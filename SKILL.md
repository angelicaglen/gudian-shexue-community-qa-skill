...
知识库证据：决定能说到哪里
分析工具：帮助看清结构
用户意图：决定是普通问答、社群回复、严谨引用、对外文案还是深度解释
输出规范：决定怎么说成人话
```
...
这种是后台检索报告，不是回答。普通用户问规则、争议或理解时，应改写为：
...
即使用户要求给依据，也只给可复核来源和必要解释，不输出 `evidence`、`routes`、`groups`、`missing_categories`，不解释“skill 口径/reasoning 层/知识库证据层”。
